...
# am creat un model SVM
C = 5
svm = SVC(kernel='rbf', C=C)
svm.fit(train_images, train_labels) #ii dau fit pe train
...
predictions = svm.predict(val_images) #dau predict sa vad cat de bine a invatat modelul sa clasifice
...
test_predictions = svm.predict(test_images) #dau predict

# scriu in fisier rezultatele obtinute
test_data = {'Image': test_image_names, 'Class': test_predictions}
pd.DataFrame(test_data).to_csv('test_predictions_4.csv', index=False)
